...
dt = Adt.dt
tfact = hbar/w0
lf, *_ = computeBaseMaps(100, 2, )
lfs = computeBaseMaps(100, 2, ret_dict=True )
N = 100
print('Data Loaded')
import polars as pl
...
NSamp = 200
N = 100
IXS = lf.select('A','B','C').explode('A').with_columns(
...
).explode('C').collect().sample(NSamp).sort('A','B','C')
...
print('Plot Made')
plt.to_image('NonLocal.png')
print('Image Made complete')